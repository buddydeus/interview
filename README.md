...
| **截图提问** | 框选当前屏幕区域，松开后由多模态模型直接读取并回答；`Esc` 取消 |
| 问最近 | 规则漏判时，把最近几段实时字幕拼起来直接问；默认取 3 条，可在设置中调整 |
| 复制 | 复制当前回答 |
...
- **截图提问**（底部按钮）：拖拽框选屏幕内容，松开后直接交给多模态模型回答。
- **问最近**（底部按钮 / `N`）：规则漏判了，把最近几句拼起来一起问；默认取 3 条，可在「设置 → 语音识别」中选择 1–8 条。

五者都**不依赖「开始监听」**：没在采集时也能用（比如手动把一句话敲进去，
或者停掉监听后想再问问刚才那句）。

截图提问要求当前模型支持 OpenAI 兼容的图片输入。`deepseek-flash` 已验证可用；
